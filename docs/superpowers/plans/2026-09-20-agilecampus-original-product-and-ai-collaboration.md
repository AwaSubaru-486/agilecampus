...
> **2026-09-20 仓库实况更新：** 本文继续作为产品架构与设计原则参考，但其中的基线提交、复选框和执行顺序已经落后于当前实现。后续开发不得按本文未勾选项从头重做；请以 [AgileCampus V2 下一阶段执行计划（仓库实况版）](./2026-09-20-agilecampus-v2-next-execution-plan.md) 为唯一施工顺序和验收依据。
...
状态列不是普通排序：把卡片拖进“待验收/已完成”会绕过提交成果和人工验收，因此状态分组看板关闭拖拽。拖拽只允许在负责人、优先级、里程碑等非状态分组中使用；服务端 `moveTaskAction` 也拒绝携带 `status` 的请求，前后端双重防护。状态推进必须走“我接手 → 提交成果 → 组长/教师验收”的动作链。
