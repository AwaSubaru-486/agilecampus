...
function errorResponse(error: unknown) {
  ...
  console.error("[context-pack] 请求失败", error);
  return NextResponse.json({ error: "读取失败，请重试" }, { status: 500 });
}
...
export async function GET(
  ...
  try {
    return NextResponse.json(await getContextPackForUser(session.user.id, packId));
  } catch (error) {
